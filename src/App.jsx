...
function App() {
       ...
       if (card.role === 'weapon') {
         ...
         if (newState.enemyHp === 0) {
            ...
            setBattleResult({
              ...
              stars: earnedStars,
              coins: reward,
              message: newState.log.slice(-1)[0],
    ...
    try {
      ...
      setInventory(prev => {
        ...
        if (existingIndex >= 0) {
          ...
          finalCardToReveal = updated[existingIndex];
          return updated;
        } else {
          return [...prev, card];
        }
  ...
  const renderVault = () => {
    ...
    return (
      <div className="view-container collection-view">
        <header className="panel-impeccable header" style={{marginBottom: '20px', display: 'flex', flexDirection: 'column', gap: '15px'}}>
          <div style={{display: 'flex', width: '100%', justifyContent: 'space-between', alignItems: 'center'}}>
             <h2 style={{color: '#fbbf24', margin: 0, textShadow: '2px 2px 0 #000'}}>The Vault</h2>
             <div className="currency" style={{border: 'none', background: 'transparent'}}>Artifacts: {visibleInventory.length} / {artifactsData.length}</div>
          </div>
          <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '1rem', color: 'var(--text-secondary)'}}>
             <span>Equipped Power: <strong style={{color: '#fff'}}>{Object.values(totalStats).reduce((sum, val) => sum + (typeof val === 'number' ? val : 0), 0)}</strong></span>
             <div style={{display: 'flex', gap: '10px', alignItems: 'center'}}>
               <button className="btn-impeccable accent" onClick={handleRandomizeDeck} style={{padding: '6px 12px', fontSize: '0.9rem', marginBottom: 0}}>🎲 Auto-Equip Random</button>
               <label style={{display: 'flex', alignItems: 'center', gap: '5px', cursor: 'pointer', background: 'rgba(0,0,0,0.8)', padding: '6px 12px', border: '2px solid #3e2723'}}>
                 <input type="checkbox" checked={showCheatSandbox} onChange={(e) => setShowCheatSandbox(e.target.checked)} />
                 <span style={{fontFamily: 'VT323', fontSize: '1.2rem'}}>Dev Sandbox</span>
               </label>
             </div>
          </div>
        </header>
  ...
  const renderSkillTree = () => (
    <div className="view-container skill-tree-view" style={{paddingBottom: '120px', maxWidth: '800px', margin: '0 auto'}}>
       <header className="panel-impeccable" style={{marginBottom: '20px', textAlign: 'center', borderColor: '#d946ef'}}>
  ...
  const renderCampaignMap = () => {
    ...
    return (
      <div className="view-container campaign-map-view panel-impeccable" style={{minHeight: '75vh', position: 'relative', marginTop: '10px', background: 'rgba(0,0,0,0.6)'}}>
        <header className="header" style={{position: 'absolute', top: 0, left: 0, width: '100%', padding: '15px', zIndex: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
          <button className="btn-impeccable secondary" onClick={() => { playClick(); setView('grand_hall'); }} style={{padding: '8px 12px', fontSize: '1rem'}}>Back to Hall</button>
          <h2 style={{textShadow: '0 2px 4px #000', margin: 0, color: '#fbbf24'}}>Campaign Map</h2>
        </header>
  ...
  const renderArenaCombat = () => {
    ...
    return (
      <div className="view-container battle-view" style={{paddingBottom: '80px'}}>
        <header className="panel-impeccable header" style={{marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
          <div style={{display: 'flex', gap: '10px'}}>
            <button className="btn-impeccable secondary" onClick={() => { playClick(); setShowFleeModal(true); }} style={{marginBottom: 0, padding: '8px 12px', fontSize: '1rem'}}>Flee to Hall</button>
            <button className={`btn-impeccable ${isAutoBattle ? 'danger' : 'secondary'}`} onClick={() => { playClick(); setIsAutoBattle(!isAutoBattle); }} style={{marginBottom: 0, padding: '8px 12px', fontSize: '1rem'}}>AUTO: {isAutoBattle ? 'ON' : 'OFF'}</button>
            {cheatMode && (
              <button 
          ...
          <h2 style={{margin: 0, textShadow: '2px 2px 0 #000'}}>Arena Combat</h2>
        </header>
    ...
    <div className={`app-container ${shakeScreen ? 'shake-active' : ''}`}>
      ...
      {view !== 'arena_combat' && view !== 'campaign_map' && (
        <>
          ...
          <header className="global-hud glass" style={{position: 'sticky', top: 0, zIndex: 100, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 15px', borderBottom: '2px solid var(--surface-border)', borderTop: 'none', borderLeft: 'none', borderRight: 'none', borderRadius: 0}}>
             ...
             <div className="top-nav-center" style={{flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '20px'}}>
